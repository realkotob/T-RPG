...
func place_tile(cell: Vector2, tile_id: int, tile_type: int) -> void:
	var tile_type_name = GAME.TILE_TYPE.keys()[tile_type]
	tile_type_name = tile_type_name.to_lower().capitalize()
	
	var target_tilemap = get_node(tile_type_name) if tile_type != GAME.TILE_TYPE.TILE  else self
	
	target_tilemap.set_cellv(cell, tile_id)
